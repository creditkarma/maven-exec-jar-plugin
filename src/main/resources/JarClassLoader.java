 ...
 * Copyright (C) 2015-2016 Credit Karma, Inc. All Rights Reserved.
 *
 ...
 * an applet. This technique is very convenient to develop an applet and 
 * test it as an application. 
 *
 * <p>
 * Use VM parameters in the command line for logging settings (examples):
 * <ul>
 * <li><code>-DJarClassLoader.logger=[filename]</code> for logging into 
 * the file.
...
public class JarClassLoader extends ClassLoader {
     ...
     * <p>
     * JarClassLoader extracts all JARs and native libraries into 
     * temporary files
     ...
     * periodically and empty it if required. Temporary files could 
     * accumulate
    ...
    public JarClassLoader(ClassLoader parent) {
        ...
        if ("http".equals(protocol) || "https".equals(protocol)) {
            // Protocol 'http' or 'https' - application launched from
	    // WebStart / JNLP or as Java applet
            try {
                // Convert:
                //   urlTopJar = "http://.../MyApp.jar" --> connection
		// sun.net.www.protocol.http.HttpURLConnection
                // to
                //   urlTopJar = "jar:http://.../MyApp.jar!/" -->
		// connection java.net.JarURLConnection
                urlTopJar = new URL("jar:" + urlTopJar + "!/");
                JarURLConnection jarCon =
		    (JarURLConnection)urlTopJar.openConnection();
                JarFile jarFile = jarCon.getJarFile();
                jarFileInfo = new JarFileInfo(jarFile, jarFile.getName(),
					      null, pdTop, null);
                logInfo(LogArea.JAR,
	    ...
	    //    "/C:/my dir/MyApp.jar" 
            try {
                sUrlTopJar =
		    URLDecoder.decode(urlTopJar.getFile(), "UTF-8");
        ...
        checkShading();
	String ignore = System.getProperty("jarClassLoader.ignoreShutdownHook");
	if (null == ignore || !"true".equalsIgnoreCase(ignore)) {
    ...
    private void initLogger() {
        ...
        if (sLogger != null) {
            try {
                ...
                logError(LogArea.CONFIG, "Cannot create log file %s.",
			 sLogger);
            }
        }
     ...
     * 6. Option "java.protocol.handler.pkgs" does not allow access to 
     * nested JARs(?).
     *
    ...
    throws JarClassLoaderException {
        ...
        File fileTmp = null;
        try {
            fileTmp = File.createTempFile(inf.getName() + ".", null,
					  dirTemp);
            fileTmp.deleteOnExit();
            chmod777(fileTmp); // Unix - allow temp file deletion by any
	    // user
    ...
    private void loadJar(JarFileInfo jarFileInfo) throws IOException {
        ...
        try {
            ...
            while (en.hasMoreElements()) {
                ...
                if (s.lastIndexOf(EXT_JAR) ==
		    s.length() - EXT_JAR.length()) {
			    ...
			    "Loading inner JAR %s from temp file %s",
                    ...
                    // 'csParent' is never null: top JAR has it, JCL
		    // creates it for child JAR:
			...
			new ProtectionDomain(csChild, 
					     pdParent.getPermissions(),
					     pdParent.getClassLoader(),
					     pdParent.getPrincipals());
                    loadJar(new JarFileInfo(
                            new JarFile(fileTemp), inf.getName(),
			    jarFileInfo, pdChild, fileTemp));
                }
            }
    ...
    private JarEntryInfo findJarNativeEntry(String sLib) {
		...
		// "abc/xyz/Native.dll"
                ...
                //   - in the path: abc/Native.dll/xyz/my.dll <-- do not
		// load this one!
                //   - in the partial name: abc/aNative.dll   <-- do not
		// load this one!
                String[] token = sEntry.split("/"); // the last token is
		// library name
                if (token.length > 0 &&
		    (token[token.length - 1].equals(sName) ||
		     // hack to work around issues with older versions of
		     // kamon that load the wrong library name
		     token[token.length - 1].equals(sLib))) {
		    logInfo(LogArea.NATIVE,
			    "Loading native library '%s' found as '%s' " +
			    "in JAR %s",
     ...
     * "[user.home]/.JarClassLoader" and delete them on next application 
     * run.
     * <p>
    ...
    private void deleteOldTemp(File fileCfg) {
        ...
        try {
            ...
            logDebug(LogArea.CONFIG,
		     "Deleted %d old temp files listed in %s",
    ...
    private void persistNewTemp(File fileCfg) {
        if (hsDeleteOnExit.size() == 0) {
            logDebug(LogArea.CONFIG,
		     "No temp file names to persist on exit.");
    ...
    public void invokeMain(String sClass, String[] args) throws Throwable {
        ...
        logInfo(LogArea.CONFIG,
		"Launch: %s.main(); Loader: %s", sClass,
        ...
        if (method == null  ||  !bValidModifiers  ||  !bValidVoid) {
            throw new NoSuchMethodException(
                    "The main() method in class \"" + sClass +
		    "\" not found.");
        }
	...
	throws Throwable {
        ...
        Class<?> clazz = loadClass(sClass);
        logInfo(LogArea.CONFIG,
		"initApplet() --> %s.init(); Loader: %s", sClass,
		clazz.getClassLoader());
    ...
    public void startApplet() {
        checkApplet();
        logInfo(LogArea.CONFIG, "startApplet() --> %s.start()",
    ...
    public void stopApplet() {
        checkApplet();
        logInfo(LogArea.CONFIG, "stopApplet() --> %s.stop()",
		applet.getClass().getName());
        applet.stop();
    }
    ...
    public void destroyApplet() {
        checkApplet();
        logInfo(LogArea.CONFIG, "destroyApplet() --> %s.destroy()",
		applet.getClass().getName());
        applet.destroy();
    }
    ...
    {
        logDebug(LogArea.CLASS, "LOADING %s (resolve=%b)",
		 sClassName, bResolve);
        // Each thread must have THIS class loader set as a context class
	// loader. 
        // This is required to prevent failure finding a class or resource
	// from  
        ...
        // 2. THIS class loader passes request to system default class
	// loader 
        // to load the class from rt.jar.
        // 3. The class 'javax.swing.JOptionPane' is loaded by system
	// default class
        // loader.
        // 4. The class 'javax.swing.JOptionPane' is requesting
	// 'UIDefaults.getUI()'
        // for component, which resides in external LnF JAR.
        // 5. The class loader which is used to load the requested
	// component is 
        // current thread context class loader if it is set, otherwise
	// the parent  
        ...
        // thread context class loader is not set. The default system
	// class loader is 
        //   - sun.misc.Launcher$AppClassLoader - run from file system or
	// JAR
	...
	// invokeMain()
        // method is sufficient for most cases. It fails for new threads
	// created
        ...
        try {
            ...
            if (isLaunchedFromJar()) {
                try {
                    c = findJarClass(sClassName); // Do not simplify!
		    // See "finally"!
                    return c;
                } catch (JarClassLoaderException e) {
                    if (e.getCause() == null) {
                        logDebug(LogArea.CLASS,
				 "Not found %s in JAR by %s: %s",
				 sClassName, getClass().getName(),
				 e.getMessage());
                    } else {
                        logDebug(LogArea.CLASS,
		    ...
		    // we are shutting down so use the parent classloader
		    // instead
		    return getParent().loadClass(sClassName);
		}
		// keep looking...
            }
            ...
            try {
                // No need to call findLoadedClass(sClassName) because
		// it's called inside:
                ClassLoader cl = getParent();
                c = cl.loadClass(sClassName);
                // System classloader does not define
		// ProtectionDomain->CodeSource - null 
                logInfo(LogArea.CLASS, "Loaded %s by %s", sClassName,
			cl.getClass().getName());
                return c;
            } catch (ClassNotFoundException e) {
            }
            // What else?
            throw new ClassNotFoundException("Failure to load: " +
					     sClassName);
        } finally {
            if (c != null  &&  bResolve) {
                resolveClass(c);
    ...
    public Enumeration<URL> findResources(String sName) throws IOException {
        ...
        if (isLaunchedFromJar()) {
            List<JarEntryInfo> lstJarEntry =
		findJarEntries(normalizeResourceName(sName));
     ...
     * package
     * for the loaded class and leaves it null. Each package referenced by 
     * this
    ...
    throws IllegalArgumentException {
        ...
        if (getPackage(sPackageName) == null) {
            ...
            definePackage(sPackageName,
			  jfi.getSpecificationTitle(),
			  jfi.getSpecificationVersion(),
			  jfi.getSpecificationVendor(),
			  jfi.getImplementationTitle(),
			  jfi.getImplementationVersion(),
			  jfi.getImplementationVendor(),
			  jfi.getSealURL());
        }
    }
	...
	// Unix: allow content for dir, redundant for file
    ...
    private void log(LogLevel level, LogArea area, String sMsg,
		     Object ... obj) {
        if (level.ordinal() <= logLevel.ordinal()) {
            if (hsLogArea.contains(LogArea.ALL) ||
		hsLogArea.contains(area)) {
	...
	// "topJar!childJar!kidJar"
         ...
         * @param jarFileParent
         *            Used to make simpleName for logging. Null for top 
	 * level JAR.
         * @param fileDeleteOnExit
         *            Used only to delete temporary file on exit. 
         *            Could be null if not required to delete on exit 
	 * (top level JAR)
         * @throws JarClassLoaderException 
         */
		...
		(jarFileParent == null ? "" :
		 jarFileParent.simpleName + "!") + simpleName;
            ...
            try {
		// 'null' if META-INF directory is missing
        ...
        String getSpecificationTitle() {
            return
		mf.getMainAttributes().getValue(Name.SPECIFICATION_TITLE);
        }
        String getSpecificationVersion() {
            return
		mf.getMainAttributes().getValue(Name.SPECIFICATION_VERSION);
        }
        String getSpecificationVendor() {
            return
		mf.getMainAttributes().getValue(Name.SPECIFICATION_VENDOR);
        }
        String getImplementationTitle() {
            return
		mf.getMainAttributes().getValue(Name.IMPLEMENTATION_TITLE);
        }
        ...
        String getImplementationVendor() {
            return
		mf.getMainAttributes().getValue(Name.IMPLEMENTATION_VENDOR);
        }
        URL getSealURL() {
        ...
        byte[] getJarBytes() throws JarClassLoaderException {
            ...
            try {
                ...
                if (lSize <= 0  ||  lSize >= Integer.MAX_VALUE) {
                    throw new JarClassLoaderException(
                            "Invalid size " + lSize + " for entry " +
			    jarEntry);
                }
                a_by = new byte[(int)lSize];
                InputStream is =
		    jarFileInfo.jarFile.getInputStream(jarEntry);
